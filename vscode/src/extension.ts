	...
	context.subscriptions.push(
		...
		vscode.commands.registerCommand(
			...
			async (uri: vscode.Uri | undefined) => {
				...
				await client?.sendNotification(lspNotificationSetSourceRoot, {
					uri: sourceRoot.toString(),
				});
			},
		),
...
function getServer(): string {
	...
	const extension = vscode.extensions.getExtension(extensionId)?.packageJSON as
		| { version: string }
		| undefined;
		...
		case "darwin":
			switch (arch()) {
				case "arm64":
					return path.join(
						serverDir,
						`tablegen-lsp-${version}-aarch64-apple-darwin`,
					);
			}
			break;
		case "linux":
			switch (arch()) {
				case "arm64":
					return path.join(
						serverDir,
						`tablegen-lsp-${version}-aarch64-unknown-linux-gnu`,
					);
				case "x64":
					return path.join(
						serverDir,
						`tablegen-lsp-${version}-x86_64-unknown-linux-gnu`,
					);
			}
			break;
		case "win32":
			switch (arch()) {
				case "arm64":
					return path.join(
						serverDir,
						`tablegen-lsp-${version}-aarch64-pc-windows-msvc.exe`,
					);
				case "x64":
					return path.join(
						serverDir,
						`tablegen-lsp-${version}-x86_64-pc-windows-msvc.exe`,
					);
			}
			break;
	}
...
function initStatusBarItem(): vscode.StatusBarItem {
	...
	const sourceRootPath = vscode.workspace
		.getConfiguration("tablegen-lsp")
		.get("defaultSourceRootPath");
